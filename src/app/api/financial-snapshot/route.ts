...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json(
      { error: "Error creating financial snapshot" },
      { status: 500 }
    );
  }
}
...
export async function GET(req: Request) {
  try {
    ...
    const snapshot = await FinancialSnapshot.findOne({ user: userId }).sort(
      "-createdAt"
    );

    if (!snapshot) {
      return NextResponse.json(
        { error: "No financial snapshot found" },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json(
      { error: "Error fetching financial snapshot" },
      { status: 500 }
    );
  }
}
